...
| 029 | 2026-06-03 | Segurança — corta `select('*')` em profiles (vazava hash) | Phase 2 | DEC-008 | PatientDashboard e CompletarPerfilModal liam a linha inteira de `profiles` pro cliente (incl. `senha_klipbit`). Trocado por lista explícita das 9 colunas usadas pela tela e pelos filhos. (commit 72fee56) |
| 030 | 2026-06-03 | Catálogo de suplementos orais — tabela + 1ª categoria | Phase 2 | DEC-010 | `migrate_suplementos.sql`: tabela ÚNICA `suplementos` (categoria, posologia, via, apresentacao, composicao, ativo, cota) amplia o catálogo além do ferro EV. Seed da 1ª categoria: 4 polivitamínicos bariátricos (Baristar [ativa], Bariat XR, Multi Bariátrico Catarinense, Complete Bari). B12/ferro oral aguardam curadoria. Sem RLS por ora. |
| 031 | 2026-06-03 | Suplementos — B12 + ferro oral + coluna `indicacao` | Phase 2 | DEC-010 | `migrate_suplementos_b12_ferro.sql`: +11 marcas (b12_sublingual MecoBe[ativa]/Dozemast; b12_injetavel Citoneurin[ativa]/Nevrix/cianocobalamina pura; b12_oral cianocobalamina pura[ativa]/Renovi B; ferro_oral Noripurum[ativa]/Neutrofer/Combiron/Folifer). Decisões: B12 combo+pura, ferro 1 linha/marca. Nova coluna `indicacao` (contexto clínico por categoria) — inclui o esquema B12 do bariátrico (IM grave → 3 doses → sublingual). Critérios de escolha ("quando") ficam no engine (futuro `b12Protocol.js`), não aqui. Vários "a confirmar" p/ revisão médica. |
