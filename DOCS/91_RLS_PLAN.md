...
**Fase 1 — Tabelas fáceis: ✅ FECHADA (2026-06-03)**
- RPCs admin `salvar_medicamento`/`salvar_config` (crachá `token_admin_ok`); AdminPage salva via RPC.
- RLS ligado (`migrate_rls_fase1_enable.sql`): `medicamentos`/`config` SELECT público + escrita só-RPC; `leads_comerciais` só INSERT.
- **Achado no caminho:** as tabelas `medicamentos` e `leads_comerciais` **nunca tinham sido criadas** no Supabase (só `config` existia) — o catálogo DEC-007 vinha degradando calado. Criadas agora (`migrate_add_medicamentos.sql` + `migrate_create_leads.sql`); catálogo finalmente vivo.
- Validado em produção: catálogo (5 drogas), modal com 2 receitas, leitura/escrita de config sob RLS — tudo ok.
