...
-- 0) Config: comissão do indicador NÃO afiliado (dólar digital; quantidade própria).
INSERT INTO public.config (chave, valor, descricao) VALUES
  ('comissao_usd_nao_afiliado', '10', 'Comissão em DÓLAR do indicador NÃO afiliado (não-médico do OBA) por paciente convertido')
...
BEGIN
  ...
  -- Comissão do INDICADOR (não afiliado): chave própria, com fallback p/ a antiga e depois 10.
  v_usd_ind := COALESCE(
