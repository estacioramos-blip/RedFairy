...
$f = 'src\components\HistoricoChartModal.jsx'
if (-not (Test-Path $f)) { Write-Host 'ERRO: rode na raiz do projeto.'; exit 1 }

# Backup leve (mantem o arquivo antigo como .bak)
$bak = $f + '.bak'
...
[System.IO.File]::WriteAllText((Resolve-Path $f), $novo, (New-Object System.Text.UTF8Encoding($false)))

# Tambem precisa atualizar a prop que o TriagemModal passa: avaliacoes -> serie
$ft = 'src\components\TriagemModal.jsx'
$ct = Get-Content $ft -Raw
$tOld = 'avaliacoes={historicoData.avaliacoes}'
$tNew = 'serie={historicoData.serie}'
if ($ct.Contains($tOld)) {
  $ct = $ct.Replace($tOld, $tNew)
  [System.IO.File]::WriteAllText((Resolve-Path $ft), $ct, (New-Object System.Text.UTF8Encoding($false)))
  Write-Host 'OK (HistoricoChartModal reescrito + prop ajustada no TriagemModal)'
} else {
  Write-Host 'PARCIAL: HistoricoChartModal reescrito, mas nao achei prop avaliacoes no TriagemModal'
}
