...
export default function LandingPage({ onModoMedico, onModoPaciente, onIrLogin, onIrDashboardPaciente, onModoAdmin, autoOBA = false }) {
  ...
  // Entrada do hero: clique na hemácia (RedFairy) ou no PLAY do splash bariátrico.
  // Médico logado entra direto; paciente logado vai pro dashboard; senão -> escolha.
  // No bariatrico.net o flag 'bariatrica' já está em localStorage (App.jsx), então
  // entrar por aqui já segue o fluxo bariátrico/OBA.
  function entrarHero() {
  ...
  return (
    <div style={{ fontFamily:"'DM Sans', sans-serif", overflowX:'hidden', maxWidth:'100vw', position:'relative' }}>
      ...
      {/* SPLASH BARIÁTRICO (bariatrico.net): porta de entrada exclusiva. Imagem
          fat/slim em tela cheia; "Projeto OBA®" amarelo translúcido no terço
          superior; PLAY (cinza piscante, ▶ amarelo) + "ENTRE" no terço inferior.
      ...
      {false && ehDominioBariatrico() && fluxoEtapa === 'inicio' && (
        <div style={{ position:'fixed', inset:0, zIndex:2000, backgroundColor:'#1a1a1a',
          ...
          <div style={{ position:'absolute', top:'40%', left:0, right:0, textAlign:'center', padding:'0 1.2rem' }}>
            <h1 style={{ margin:0, fontWeight:900, lineHeight:1.05, fontSize:'clamp(2.2rem, 9vw, 4rem)',
              color:'#FFE000', textShadow:'0 2px 14px rgba(0,0,0,0.6), 0 0 4px rgba(0,0,0,0.45)', letterSpacing:'0.5px' }}>
              {"Projeto OBA"}<sup style={{ fontSize:'0.42em', verticalAlign:'super' }}>{"®"}</sup>
            </h1>
            <p style={{ margin:'0.5rem 0 0', fontWeight:800, fontSize:'clamp(0.95rem, 4vw, 1.4rem)',
              color:'#FFE000', textShadow:'0 1px 8px rgba(0,0,0,0.6)' }}>
              {"Otimizar o Bariátrico"}
            </p>
          </div>
          <div style={{ position:'absolute', bottom:'13%', left:0, right:0, display:'flex', justifyContent:'center' }}>
            <PlayButton onClick={entrarHero} label="ENTRE" ariaLabel="Entrar"
              circleClass="bg-gray-700 hover:bg-gray-800"
              playColor="#FFE000" labelColor="#FFE000" ringColor="rgba(255,224,0,0.6)" />
          </div>
        </div>
      )}
                    ...
                    <>
                      <div style={{ position:'relative' }}>
                        ...
                        <button type="button" onClick={() => setCaixaShowSenha(s => !s)}
                          ...
                          <span style={{ fontSize:'17px', lineHeight:1 }}>{caixaShowSenha ? '🙈' : '👁'}</span>
                        </button>
                      </div>
                      <p style={{ fontSize:'0.62rem', color:'#6B7280', fontWeight:700, letterSpacing:'1px', margin:'6px 0 12px', textAlign:'center' }}>LOGIN NO SISTEMA</p>
                      {caixaModo === 'cadastro' && (
                    ...
                    <>
                      <div style={{ position:'relative' }}>
                        ...
                        <button type="button" onClick={() => setCpfPacShowSenha(s => !s)}
                          ...
                          style={{ position:'absolute', right:'10px', top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', padding:'4px', color:'#9ca3af' }}>
                          <span style={{ fontSize:'17px', lineHeight:1 }}>{cpfPacShowSenha ? '🙈' : '👁'}</span>
                        </button>
                      </div>
